...
library(readxl)
library(gifski)
total<- read_excel("~/Desktop/Master/S2/Time Series ECOM90004/Group Project /Data1/total.xlsx")
...
.center[![](spillover.png)]

--
.left[
- $VD_{ij}$: represents the contribution to the variance of the ith variable from a shock in the jth variable 
- $VD_{ii}$: represents the contribution to the variance of the ith variable from its own shocks]
--
.left[
- By subtracting out the effects of each variables own shocks, the spillover index gives the percentage of total variability in the system that is due to spillovers between markets.     
- The strategy in the following section is to compute and compare the spillover indexes for three different forecast horizons in the pre-crisis, crisis and post-crisis periods.     
]
